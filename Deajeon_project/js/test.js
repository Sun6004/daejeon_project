...
function renderCourseTimeline() {
  ...
  const items = stops
    .map((stop, i) => {
      ...
      // stop.url 값을 데이터에 직접 넣으면 그 링크로 연결됩니다. (없으면 구글 지도 검색으로 대체)
      const linkUrl = stop.url || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent("대전 " + stop.name)}`;
      return `
      <div class="timeline-item">
        <div class="timeline-num" style="background:${tc.soft};border:2px solid ${tc.light};color:${tc.main};">${i + 1}</div>
        <a class="timeline-card" href="${linkUrl}" target="_blank" rel="noopener noreferrer" style="background:${tc.soft};">
...
function renderFamousGrid() {
  ...
  el.innerHTML = cat.places
    .map((p) => {
      // p.url 값을 데이터에 직접 넣으면 그 링크로 연결됩니다. (없으면 구글 지도 검색으로 대체)
      const linkUrl = p.url || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent("대전 " + p.name)}`;
      return `
    <a class="famous-card" href="${linkUrl}" target="_blank" rel="noopener noreferrer">
...
// 발급된 패스 이미지 파일 경로 (공유 시 이 이미지를 함께 첨부)
...
function renderIssuedPass() {
  ...
  if (type === "힐링") {
    ...
    imageAreaHtml = `<img src="images/pass-activity.png" alt="액티비티 패스" />`;
  } else {
    imageAreaHtml = `<div class="pass-placeholder">
      ${ico(TYPE_ICON[type], 56)}
      <p class="pass-placeholder-title" style="color:${tc.main};">${type} 패스</p>
  ...
  async function handleShare() {
    const shareTitle = `대전 ${type} 패스 발급 완료!`;
    const shareText = `나는 대전 ${type} 패스를 받았어! 같이 대전 가자 🎫`;
    const shareUrl = "https://daejeon-project-82zvgb2z0-spectrum11-fd26.vercel.app/";
    const imagePath = PASS_IMAGE_PATH[type];

    // 1) 패스 이미지 파일 + 링크 공유 시도 (모바일 Web Share API, 카톡 등 공유 시트)
    if (imagePath && navigator.share && navigator.canShare) {
      try {
        const res = await fetch(imagePath);
        const blob = await res.blob();
        const file = new File([blob], `daejeon-${type}-pass.png`, { type: blob.type || "image/png" });
        if (navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: shareTitle, text: `${shareText}\n${shareUrl}` });
          return;
        }
      } catch (e) {
        // 이미지 공유 실패 시 아래 텍스트 공유로 자동 폴백
      }
    }

    // 2) 이미지 공유가 안 되는 환경: 텍스트 + 링크만 공유
    if (navigator.share) {
      try {
        await navigator.share({ title: shareTitle, text: shareText, url: shareUrl });
        return;
      } catch (e) {
        return; // 사용자가 공유를 취소한 경우 무시
      }
    }

    // 3) Web Share API 자체가 없는 PC 브라우저: 클립보드 복사
